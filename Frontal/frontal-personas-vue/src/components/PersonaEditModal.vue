<template>
  <dialog ref="dlg" class="personas-modal" @keydown.esc.prevent="close" @click="onBackdrop">
    <Form :validation-schema="schema" @submit="onSubmit" v-if="persona">
      <h3>Editar Persona</h3>
      <div class="field">
        <label for="nombre">Nombre</label>
        <Field id="nombre" name="nombre" :model-value="persona.nombre" as="input" />
        <ErrorMessage name="nombre" class="error" />
      </div>
      <div class="field">
        <label for="dni">DNI</label>
        <Field id="dni" name="dni" :model-value="persona.dni" as="input" />
        <ErrorMessage name="dni" class="error" />
      </div>
      <div class="field">
        <label for="email">Email</label>
        <Field id="email" name="email" :model-value="persona.email" type="email" as="input" />
        <ErrorMessage name="email" class="error" />
      </div>
      <div class="field">
        <label for="edad">Edad</label>
        <Field id="edad" name="edad" :model-value="persona.edad" type="number" as="input" />
...
const emit = defineEmits<(e:'update', id:string, data: ModificarPersona)=>void>();
const props = defineProps<{ open: boolean; persona: Persona | null }>();
...
function onSubmit(values: any) {
  if (props.persona) emit('update', props.persona.id, values as ModificarPersona);
  close();
}
function open() { if (props.persona) dlg.value?.showModal(); }
function close() { dlg.value?.close(); }
function onBackdrop(e: MouseEvent) { if (e.target === dlg.value) close(); }

watch(() => props.open, (val) => { if (val) open(); else close(); });
</script>
<style scoped>
